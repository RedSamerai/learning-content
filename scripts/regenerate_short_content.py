...
# 建立KG索引
kg_data = json.load(open(KG_FILE, encoding='utf-8'))
kg_index = {}
for item in kg_data:
    key = f"{item.get('grade', '')}_{item.get('subject', '')}_{item.get('id', '')}"
    kg_index[key] = item
...
def main():
    ...
    for f in OUTPUT_DIR.rglob('*.json'):
        ...
        try:
            ...
            # 检查是否是AI拒绝回答的内容
            if '我注意到您提供的知识点存在学科矛盾' in content:
                print(f"发现异常内容: {f.name}")
            if len(content) < 300:
                short_files.append({
                    'path': f,
                    'data': data,
    ...
    for i, item in enumerate(short_files, 1):
        ...
        if new_content and len(new_content) >= 300:
            ...
            data['content'] = new_content  # 同时填充content字段
