...
async def call_api(prompt):
    ...
    data = {"model": "agnes-2.5-flash", "messages": [{"role": "user", "content": prompt}], "max_tokens": 1000}
...
async def process_topic(topic_path, subject, grade):
    ...
    try:
        ...
        print(f"[{subject}/{grade}] {topic_name}...", end=" ", flush=True)
        
        # 生成详细讲解
        prompt = f"""你是{grade}的{subject}老师。请为以下知识点生成详细的教科书式讲解：

知识点：{topic_name}
学习重点：{focus}

要求：
1. 用通俗易懂的语言解释概念（200-400字）
2. 包含2-3个生活例子帮助理解
3. 给出核心公式或方法（如有）
4. 提醒常见错误
5. 使用markdown格式，段落清晰
...
只返回讲解内容，不要其他文字。"""
        ...
        if explanation and len(explanation) > 100:
            ...
            await generate_audio(explanation[:800], audio_path, voice)
...
async def main():
    subjects = ["math", "chinese", "english", "physics", "chemistry", "biology", "history", "geography", "morality"]
    grades = [
        "一年级上", "一年级下", "二年级上", "二年级下",
        "三年级上", "三年级下", "四年级上", "四年级下",
        "五年级上", "五年级下", "六年级上", "六年级下",
    ...
    for p in Path("output").rglob("*"):
        if p.is_dir() and p.name not in ["image", "audio"]:
            ...
            if len(parts) >= 3 and parts[0] in subjects and parts[1] in grades:
                json_path = p / f"{p.name}.json"
                if json_path.exists():
                    topics.append((p, parts[0], parts[1]))
    
    print(f"找到 {len(topics)} 个知识点待生成内容\n")
