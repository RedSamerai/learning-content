...
async def generate_audio(topic_id, topic_name, grade):
    """生成音频"""
    try:
        voice = "zh-CN-XiaoxiaoNeural" if grade in ["幼儿园", "幼小衔接"] else "zh-CN-YunxiNeural"
...
async def process_topic(topic):
    """处理单个知识点"""
    topic_id = topic["id"]
    output_dir = Path(f"output/{topic['grade']}/{topic['subject']}/{topic_id}")
    output_dir.mkdir(parents=True, exist_ok=True)
    
    json_path = output_dir / f"{topic_id}.json"
    ...
    # 生成音频
    print("  🔊 生成音频...")
    audio_path = await generate_audio(topic_id, topic["topic"], topic["grade"])
...
async def main():
    # 加载知识点清单
    with open("knowledge_graph_textbook.json", "r", encoding="utf-8") as f:
        topics = json.load(f)
    ...
    for i, topic in enumerate(topics):
        ...
        if (i + 1) % 50 == 0:
            ...
            with open("progress.log", "a", encoding="utf-8") as f:
                f.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} - 进度: {completed}/{i+1}\n")
        
        # RPM控制
        await asyncio.sleep(REQUEST_DELAY)
    
    print(f"\n✅ 全部完成: {completed}/{len(topics)}")
