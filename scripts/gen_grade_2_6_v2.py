...
REQUEST_DELAY = 3.5  # 每次请求间隔3.5秒（确保低于20 RPM）
...
async def gen_audio(topic_name, grade, voice):
    try:
        ...
        audio_path = f"audio/intro_{int(time.time())}.mp3"
...
async def main():
    ...
    print(f"⏱️ 预计时间: {len(tasks) * 3.5 / 60:.1f} 分钟\n")
