...
def find_missing_audio():
    ...
    for grade in AGE_RANGE:
        ...
        for json_file in grade_dir.rglob('*.json'):
            # 检查是否有对应的音频文件
            mp3_same_dir = json_file.with_suffix('.mp3')
            audio_subdir = json_file.parent / 'audio'
            audio_mp3 = audio_subdir / 'audio.mp3'
            
            if not mp3_same_dir.exists() and not audio_mp3.exists():
                try:
                    grade_from_path = json_file.parts[2]
                except:
                    grade_from_path = grade
                
                missing.append({
                    'json_path': json_file,
                    'grade': grade_from_path,
                })
    
    return missing

def generate_audio_sync(text: str, output_path: Path, voice: str, rate: str = "-10%"):
    """同步生成音频"""
    asyncio.run(edge_tts.Communicate(text, voice, rate=rate).save(str(output_path)))
    ...
    for grade in AGE_RANGE:
        ...
        for i, item_info in enumerate(items):
            try:
                ...
                # 生成音频
                generate_audio_sync(text, mp3_file, voice)
        ...
        print(f"\n{grade} 完成: {grade_success}/{len(items)} 个音频生成成功")
